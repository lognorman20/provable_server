...
// create function
let myCall = contract.populate('create_file', ['cheese', 0x1]); // ['path', content_hash]
let res = await contract.create_file(myCall.calldata);
await provider.waitForTransaction(res.transaction_hash);

// update function
myCall = contract.populate('update', ['butter', 0x1, 0x2]); // ['path', prev_content_hash, new_content_hash]
res = await contract.update(myCall.calldata);
await provider.waitForTransaction(res.transaction_hash);

// remove function
myCall = contract.populate('remove', [0x2]); // 0x2 represents the file's content hash
res = await contract.remove(myCall.calldata);
await provider.waitForTransaction(res.transaction_hash);
